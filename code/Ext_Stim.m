%----------------------------------------------------------------------
%                        Extracting Stimulus 
%                       Version 2 - 1395/05/16
%                    Prepared by: Athena Taymourtash 
%                 for EEG Data of Amin Behzadnia MSc project
%----------------------------------------------------------------------

function [trig_on,type,stim_tar,stim_non,all_isi]=Ext_Stim(path_log,Dnam_log,SaveFile_Address_S,cid,modality)
...
for i=1:L
    C = textscan(fid, '%s ',10);
    trig_on(i,1) = str2num(C{1,1}{1,1}(1:end-1));
    Charac = (C{1,1}{2,1}(1:end-1));
    type(i,1) = str2num(Charac);
...
s_10=find(ISI>=1.1000 & ISI<=1.2080);
s_15=find(ISI>=1.6000 & ISI<=1.7080);
s_20=find(ISI>=2.1000 & ISI<=2.2080);
s_25=find(ISI>=2.6000 & ISI<=2.7080);
...
stim_tar = S;
stim_non = NS;
all_isi = [s_10;s_15;s_20;s_25];

save ([SaveFile_Address_S, '\Stim_CPT_', modality,'_',cid,'.mat'],'stim_tar','stim_non',...

...
for Subject_Num = CID %[3,5:6,8:10,12,15:22,24:25,27:29]
    ...
    amp_n2 = zeros(3,1);  %three channels: Pz, Cz and Fz \ two states: Target and Non-Target
    lat_n2 = zeros(3,1);
    ...
    for i = 1:2
        ...
        disp(['N200 estimation is started...'])
        ...
        for j = 1:2
          ...
          h=  figure(counter)
            ...
            for n = 1:3
                ...
                %% N200  Computation
                locs = [];
            for k = 88:138    % N200 interval: 250-450ms after stimulus onset
            ...
            [val pos] = min(filt_erp(loc(n),locs));
            amp_n2(n,1) = val - mean(filt_erp(loc(n),1:25));
            lat_n2(n,1) = (round(locs(pos)*(1000/fs)) - 100);
            mean_100(n,1)= mean(filt_erp(loc(n),1:25));
            
            N200 = plot3(locs(pos),filt_erp(loc(n),locs(pos)),val,'.','MarkerSize',20,'markeredgecolor','r');
            hold on
            datacursormode on
            disp(['LN2 ',modality{i,1},'_',states{j,1},'_',Channels{1,n},' (red bullet):',num2str(lat_n2(n,1)),' ms\ ',num2str(locs(pos)),' Sample'])
            usr_confirm = input([modality{i,1},'_',states{j,1},': proposed location for N200(Enter:Accept the results)?']);
            if ~isempty(usr_confirm)
                amp_n2(n,1) = filt_erp(loc(n),usr_confirm) - mean(filt_erp(loc(n),1:25));
                lat_n2(n,1) = round((usr_confirm .* (1000/fs)) - 100);
                X = ['Amplitude_N200: ',num2str(amp_n2(n,1)), '   Latency_N200: ',num2str(lat_n2(n,1)),' milisecond'];
                disp(X)
                disp('-----------------------------------------------------------------------')
            elseif(isempty(usr_confirm))
                disp('you have confirmed the result of automatic N200 extraction (red bullet)')
                disp('-----------------------------------------------------------------------')
            end
            datacursormode off
            hold off
            ...
            counter = counter + 1;
            save([SaveFile_Address_S,'\N200_',modality{i,1},'_',states{j,1},'_',cid,'.mat'],'amp_n2','lat_n2','mean_100')
            saveas(h,[SaveFile_Address_S,'\ERP_n2',modality{i,1},'_',states{j,1},'_',cid,'.fig'])
        end
    end
end
